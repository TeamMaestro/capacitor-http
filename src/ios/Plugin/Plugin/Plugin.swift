...
public class HttpPlugin: CAPPlugin {
    ...
    @objc func setBasicAuth(_ call: CAPPluginCall){
        ...
        basicAuthData[hostUri.host!] = Request.authorizationHeader(user: username, password: password)?.value
        call.resolve()
    }
    ...
    @objc func getBasicAuthHeader(_ call: CAPPluginCall){
        ...
        var auth: String?
        if(host == "*"){
            auth =  basicAuthData[host!]
        }else{
            let hostUri = URL.init(string: host!)!;
            auth =  basicAuthData[hostUri.host!]
        }
    ...
    @objc func getHeaders(_ call: CAPPluginCall) {
        ...
        if(host == "*"){
            ...
            let hostUri = URL.init(string: host)!;
            currentHeaders = appHeaders[hostUri.host!]!
        }
    ...
    @objc func request(_ call: CAPPluginCall) {
        ...
        for (key,value) in basicAuthData {
            if(key == "*"){
                headers["Authorization"] = value
            }
            if(key.contains((hostURL?.host)!)){
                headers["Authorization"] = value;
            }
        }
